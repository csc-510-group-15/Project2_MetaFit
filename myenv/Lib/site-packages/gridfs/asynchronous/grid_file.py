        ...
        if data:
            self._buffered_docs.append(
                {"files_id": self._file["_id"],
                    "n": self._chunk_number, "data": data}
            )
    ...
    async def write(self, data: Any) -> None:
        ...
        try:
            ...
            if not isinstance(data, (str, bytes)):
                raise TypeError(
                    "can only write strings or file-like objects") from None
...
class AsyncGridOut(GRIDOUT_BASE_CLASS):  # type: ignore
    ...
    async def _read_size_or_line(self, size: int = -1, line: bool = False) -> bytes:
        ...
        while received < size:
            ...
            if line:
                pos = buf.find(NEWLN, chunk_start,
                               chunk_start + needed) - chunk_start
                if pos >= 0:
