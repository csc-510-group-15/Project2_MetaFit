...
def _raw_to_dict(
    ...
    return cast(
        _T, _elements_to_dict(data, view, position, obj_end,
                              opts, result, raw_array=raw_array)
    )
...
def _encode_dbref(name: bytes, value: DBRef, check_keys: bool, opts: CodecOptions[Any]) -> bytes:
    ...
    if value.database is not None:
        buf += _name_value_to_bson(b"$db\x00",
                                   value.database, check_keys, opts)
...
def _encode_list(
    ...
    data = b"".join([_name_value_to_bson(next(lname), item,
                    check_keys, opts) for item in value])
...
def _encode_int(name: bytes, value: int, dummy0: Any, dummy1: Any) -> bytes:
    ...
    if -2147483648 <= value <= 2147483647:
        ...
        try:
            ...
            raise OverflowError(
                "BSON can only handle up to 8-byte ints") from None
...
def _dict_to_bson(
    ...
    try:
        ...
        if top_level and "_id" in doc:
            elements.append(_name_value_to_bson(
                b"_id\x00", doc["_id"], check_keys, opts))
        for key, value in doc.items():
            if not top_level or key != "_id":
                elements.append(_element_to_bson(key, value, check_keys, opts))
    except AttributeError:
        raise TypeError(
            f"encoder expected a mapping type but got: {doc!r}") from None
        ...
        while position < end:
            ...
            if use_raw:
                ...
                docs.append(_elements_to_dict(
                    data, view, position + 4, obj_end, opts))
            position += obj_size
        return docs
    except InvalidBSON:
        raise
