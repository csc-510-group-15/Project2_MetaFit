...
def validate_email_local_part(local: str, allow_smtputf8: bool = True, allow_empty_local: bool = False,
    ...
    if valid:
        ...
        try:
            ...
            raise EmailSyntaxError(
                "The email address contains an invalid character.") from e
...
def validate_email_domain_name(domain: str, test_environment: bool = False, globally_deliverable: bool = True) -> DomainNameValidationResult:
    ...
    try:
        ...
        raise EmailSyntaxError(
            f"The part after the @-sign contains invalid characters ({e}).") from e
    ...
    if bad_chars:
        raise EmailSyntaxError(
            "The part after the @-sign contains invalid characters after Unicode normalization: " + ", ".join(sorted(bad_chars)) + ".")
    ...
    if DOT_ATOM_TEXT_HOSTNAME.match(domain):
        ...
        try:
            ...
            raise EmailSyntaxError(
                f"The part after the @-sign is invalid ({e}).") from e
    ...
    try:
        ...
        raise EmailSyntaxError(
            f"The part after the @-sign is not valid IDNA ({e}).") from e
    ...
    try:
        ...
        raise EmailSyntaxError(
            f"The part after the @-sign became invalid after normalizing to international characters ({e}).") from e
...
def validate_email_length(addrinfo: ValidatedEmail) -> None:
    ...
    addresses_to_check = [
        ...
        ((addrinfo.ascii_local_part or addrinfo.local_part or "") + "@" +
         addrinfo.ascii_domain, "when the part after the @-sign is converted to IDNA ASCII"),
    ]
...
def validate_email_domain_literal(domain_literal: str) -> DomainLiteralValidationResult:
    ...
    if domain_literal.startswith("IPv6:"):
        try:
            ...
            raise EmailSyntaxError(
                f"The IPv6 address in brackets after the @-sign is not valid ({e}).") from e
