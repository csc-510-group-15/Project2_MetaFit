...
class BaseScheduler(metaclass=ABCMeta):
    ...
    def _configure(self, config):
        ...
        self.timezone = astimezone(config.pop(
            "timezone", None)) or get_localzone()
        self.jobstore_retry_interval = float(
            config.pop("jobstore_retry_interval", 10))
    ...
    def _real_add_job(self, job, jobstore_alias, replace_existing):
        ...
        self._logger.info('Added job "%s" to job store "%s"',
                          job.name, jobstore_alias)
    ...
    def _create_plugin_instance(self, type_, alias, constructor_kwargs):
        ...
        try:
            ...
            if alias in plugin_container:
                ...
                raise LookupError(
                    f'No {type_} by the name "{alias}" was found')
