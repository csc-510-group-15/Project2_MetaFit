...
try:
    ...
    raise ImportError(
        "RethinkDBJobStore requires rethinkdb installed") from exc
...
class RethinkDBJobStore(BaseJobStore):
    ...
    def start(self, scheduler, alias):
        ...
        if "next_run_time" not in self.r.table(self.table_name).index_list().run(
            ...
            self.r.table(self.table_name).index_create(
                "next_run_time").run(self.conn)

        self.table = self.r.db(self.database).table(self.table_name)

    def lookup_job(self, job_id):
        results = list(self.table.get_all(
            job_id).pluck("job_state").run(self.conn))
    ...
    def _get_jobs(self, predicate=None):
        ...
        query = (
            self.table.filter(
                self.r.row["next_run_time"] != None).filter(predicate)
            if predicate
            else self.table
        )
