...
try:
    ...
    raise ImportError(
        "SQLAlchemyJobStore requires SQLAlchemy installed") from exc
...
class SQLAlchemyJobStore(BaseJobStore):
    ...
    def lookup_job(self, job_id):
        selectable = select(self.jobs_t.c.job_state).where(
            self.jobs_t.c.id == job_id)
    ...
    def _get_jobs(self, *conditions):
        ...
        selectable = selectable.where(
            and_(*conditions)) if conditions else selectable
        failed_job_ids = set()
        with self.engine.begin() as connection:
