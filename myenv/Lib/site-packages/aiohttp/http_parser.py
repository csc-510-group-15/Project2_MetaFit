...
class HeadersParser:
    ...
    def parse_headers(
        ...
        while line:
            ...
            continuation = self._lax and line and line[0] in (
                32, 9)  # (' ', '\t')
...
class HttpParser(abc.ABC, Generic[_MsgT]):
    ...
    def parse_headers(
        ...
        bad_hdr = next((h for h in singletons if len(
            headers.getall(h, ())) > 1), None)
...
class HttpRequestParser(HttpParser[RawRequestMessage]):
    ...
    def parse_message(self, lines: List[bytes]) -> RawRequestMessage:
        ...
        if len(path) > self.max_line_size:
            raise LineTooLong(
                "Status line is too long", str(
                    self.max_line_size), str(len(path))
            )
...
class HttpResponseParser(HttpParser[RawResponseMessage]):
    ...
    def parse_message(self, lines: List[bytes]) -> RawResponseMessage:
        ...
        if len(reason) > self.max_line_size:
            raise LineTooLong(
                "Status line is too long", str(
                    self.max_line_size), str(len(reason))
            )
