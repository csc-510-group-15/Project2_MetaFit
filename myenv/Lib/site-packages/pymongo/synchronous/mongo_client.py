...
class MongoClient(common.BaseObject, Generic[_DocumentType]):
    ...
    def _close_cursor_now(
        ...
        try:
            if conn_mgr:
                ...
                self._kill_cursors([cursor_id], address,
                                   self._get_topology(), session)
        except PyMongoError:
    ...
    def _kill_cursors(
        ...
        if address:
            ...
            server = topology.select_server_by_address(
                tuple(address), _Op.KILL_CURSORS)  # type: ignore[arg-type]
        else:
    ...
    def _process_kill_cursors(self) -> None:
        ...
        if address_to_cursor_ids:
            ...
            for address, cursor_ids in address_to_cursor_ids.items():
                try:
                    self._kill_cursors(cursor_ids, address,
                                       topology, session=None)
                except Exception as exc:
                    if isinstance(exc, InvalidOperation) and self._topology._closed:
                        raise
                    else:
