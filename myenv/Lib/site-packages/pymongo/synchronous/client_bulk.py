...
class _ClientBulk:
    ...
    def write_command(
        ...
        try:
            ...
            reply = bwc.conn.write_command(request_id, msg, bwc.codec)
            ...
            self.client._process_response(
                reply, bwc.session)  # type: ignore[arg-type]
            ...
            if isinstance(exc, OperationFailure):
                self.client._process_response(
                    exc.details, bwc.session)  # type: ignore[arg-type]
            else:
                self.client._process_response(
    ...
    def unack_write(
        ...
        try:
            # type: ignore[func-returns-value, misc, override]
            result = bwc.conn.unack_write(msg, bwc.max_bson_size)
            duration = datetime.datetime.now() - bwc.start_time
            if result is not None:
                reply = _convert_write_result(
                    bwc.name, cmd, result)  # type: ignore[arg-type]
            else:
            ...
            duration = datetime.datetime.now() - bwc.start_time
            if isinstance(exc, OperationFailure):
                failure: _DocumentOut = _convert_write_result(
                    bwc.name, cmd, exc.details)  # type: ignore[arg-type]
    ...
    def _execute_batch_unack(
        ...
        request_id, msg, to_send_ops, to_send_ns = bwc.batch_command(
            cmd, ops, namespaces)
        self.unack_write(bwc, cmd, request_id, msg, to_send_ops,
                         to_send_ns, self.client)  # type: ignore[arg-type]
    ...
    def _execute_batch(
        ...
        result = self.write_command(
            bwc, cmd, request_id, msg, to_send_ops, to_send_ns, self.client)  # type: ignore[arg-type]
                ...
                for doc in cmd_cursor:
                    ...
                    if doc["ok"] and self.verbose_results:
                        if op_type == "insert":
                            ...
                            res = InsertOneResult(
                                inserted_id, acknowledged=True)
                        if op_type in ["update", "replace"]:
                            op_type = "update"
                            # type: ignore[assignment]
                            res = UpdateResult(
                                doc, acknowledged=True, in_client_bulk=True)
                        if op_type == "delete":
                            # type: ignore[assignment]
                            res = DeleteResult(doc, acknowledged=True)
    ...
    def _execute_command(
        ...
        while self.idx_offset < self.total_ops:
            ...
            if write_concern.acknowledged:
                raw_result, to_send_ops, _ = self._execute_batch(
                    bwc, cmd, ops, namespaces)  # type: ignore[arg-type]
                result = raw_result
                ...
                if result.get("error"):
                    ...
                    if retryable and (retryable_top_level_error or retryable_network_error):
                        ...
                        _merge_command(self.ops, self.idx_offset,
                                       full_result, result)
                        _throw_client_bulk_write_exception(
