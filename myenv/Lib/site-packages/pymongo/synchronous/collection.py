...
class Collection(common.BaseObject, Generic[_DocumentType]):
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> NoReturn:
        ...
        raise TypeError(
            ...
            "failing because no such method exists." % self._name.split(
                ".")[-1]
        )
    ...
    def _create(
        ...
        if encrypted_fields:
            ...
            self._create_helper(_ecoc_coll_name(
                encrypted_fields, self._name), opts, None, session)
            self._create_helper(
    ...
    def bulk_write(
        ...
        blk = _Bulk(self, ordered, bypass_document_validation,
                    comment=comment, let=let)
        for request in requests:
            try:
                request._add_to_bulk(blk)
            except AttributeError:
                raise TypeError(
                    f"{request!r} is not a valid request") from None
    ...
    def _update(
        ...
        command = {"update": self.name,
                   "ordered": ordered, "updates": [update_doc]}
        if let is not None:
    ...
    def _delete(
        ...
        command = {"delete": self.name,
                   "ordered": ordered, "deletes": [delete_doc]}
    ...
    def _list_indexes(
        ...
        coll = cast(
            ...
            self.with_options(codec_options=codec_options,
                              read_preference=ReadPreference.PRIMARY),
        )
        ...
        def _cmd(
            ...
            try:
                cursor = (
                    self._command(conn, cmd, read_preference,
                                  codec_options, session=session)
                )["cursor"]
    ...
    def _create_search_indexes(
        ...
        **kwargs: Any,
        ...
        cmd = {"createSearchIndexes": self.name,
    ...
    def update_search_index(
        ...
        **kwargs: Any,
        ...
        .. versionadded:: 4.5
        """
        cmd = {"updateSearchIndex": self._name,
    ...
    def options(
        ...
        cursor = dbo.list_collections(session=session, filter={
                                      "name": self._name}, comment=comment)
    ...
    def _find_and_modify(
        ...
        **kwargs: Any,
        ...
        collation = validate_collation_or_none(kwargs.pop("collation", None))
        cmd = {"findAndModify": self._name,
               "query": filter, "new": return_document}
        if let is not None:
            common.validate_is_mapping("let", let)
            cmd["let"] = let
        cmd.update(kwargs)
