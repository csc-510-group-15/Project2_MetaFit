...
class _Bulk:
    ...
    def write_command(
        ...
        try:
            reply = bwc.conn.write_command(
                request_id, msg, bwc.codec)  # type: ignore[misc]
            ...
            # type: ignore[arg-type]
            client._process_response(reply, bwc.session)
            ...
            if isinstance(exc, (NotPrimaryError, OperationFailure)):
                # type: ignore[arg-type]
                client._process_response(exc.details, bwc.session)
            raise
    ...
    def unack_write(
        ...
        try:
            ...
            result = bwc.conn.unack_write(msg, max_doc_size)
            duration = datetime.datetime.now() - bwc.start_time
    ...
    def _execute_batch_unack(
        ...
        if self.is_encrypted:
            ...
            self.unack_write(bwc, cmd, request_id, msg, 0,
                             to_send, client)  # type: ignore[arg-type]
    ...
    def _execute_batch(
        ...
        if self.is_encrypted:
            ...
            result = self.write_command(
                bwc, cmd, request_id, msg, to_send, client)  # type: ignore[arg-type]
