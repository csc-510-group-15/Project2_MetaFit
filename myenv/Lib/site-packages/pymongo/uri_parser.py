    ...
    if tlsinsecure is not None:
        for opt in _IMPLICIT_TLSINSECURE_OPTS:
            if opt in options:
                ...
                raise InvalidURI(
                    err_msg % (options.cased_key("tlsinsecure"),
...
def split_options(
    ...
    if validate:
        options = cast(_CaseInsensitiveDictionary,
                       validate_options(options, warn))
...
_ALLOWED_TXT_OPTS = frozenset(
    ["authsource", "authSource", "replicaset",
        "replicaSet", "loadbalanced", "loadBalanced"]
)
