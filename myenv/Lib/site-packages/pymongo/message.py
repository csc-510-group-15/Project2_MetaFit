    ...
    if errmsg:
        ...
        if result.get("wtimeout"):
            res["writeConcernError"] = {
                "errmsg": errmsg, "code": 64, "errInfo": {"wtimeout": True}}
        else:
            # The write failed.
            error = {"index": 0, "code": result.get(
                "code", 8), "errmsg": errmsg}
...
def _gen_find_command(
    ...
    if options:
        cmd.update([(opt, True)
                   for opt, val in _OPTIONS.items() if options & val])
...
def _op_msg_no_header(
    ...
    if identifier and docs is not None:
        ...
        data = [flags_type, encoded, type_one,
                encoded_size, cstring, *encoded_docs]
    else:
...
class _EncryptedBulkWriteContext(_BulkWriteContext):
    ...
    def batch_command(
        ...
        outgoing = _inflate_bson(memoryview(
            msg)[cmd_start:], DEFAULT_RAW_BSON_OPTIONS)
...
def _client_batched_op_msg_impl(
    ...
    # document size client-side or applications won't be notified.
    if not ack:
        _check_doc_size_limits("bulkWrite", len(
            command_encoded), max_bson_size + _COMMAND_OVERHEAD)
    ...
    for (real_op_type, op_doc), namespace in zip(operations, namespaces):
        ...
        if real_op_type == "replace":
            op_type = "update"
            if not ack:
                doc_size = len(_dict_to_bson(
                    op_doc["updateMods"], False, opts))
                _check_doc_size_limits(real_op_type, doc_size, max_bson_size)
        ...
        if self.flags & 1:
            ...
            if error_object["$err"].startswith(HelloCompat.LEGACY_ERROR):
                ...
                raise ExecutionTimeout(
                    error_object.get("$err", default_msg), error_object.get(
                        "code"), error_object
                )
...
class _OpMsg:
    ...
    def raw_response(
        ...
        inflated_response = bson._decode_selective(
            RawBSONDocument(
                self.payload_document), user_fields, _RAW_ARRAY_BSON_OPTIONS
        )
    ...
    def as_command(
        ...
        if self.session:
            ...
            if not explain and not self.session.in_transaction:
                self.session._update_read_concern(
                    cmd, conn)  # type: ignore[arg-type]
        # type: ignore[arg-type]
