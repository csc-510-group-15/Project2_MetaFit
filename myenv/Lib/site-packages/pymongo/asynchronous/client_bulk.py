...
class _AsyncClientBulk:
    ...
    async def write_command(
        ...
        try:
            ...
            reply = await bwc.conn.write_command(request_id, msg, bwc.codec)
    ...
    async def unack_write(
        ...
        try:
            ...
            result = await bwc.conn.unack_write(msg, bwc.max_bson_size)
            duration = datetime.datetime.now() - bwc.start_time
    ...
    async def _execute_batch_unack(
        ...
        request_id, msg, to_send_ops, to_send_ns = bwc.batch_command(
            cmd, ops, namespaces)
        # type: ignore[arg-type]
        await self.unack_write(bwc, cmd, request_id, msg, to_send_ops, to_send_ns, self.client)
    ...
    async def _execute_command(
        ...
        while self.idx_offset < self.total_ops:
            ...
            if write_concern.acknowledged:
                # type: ignore[arg-type]
                raw_result, to_send_ops, _ = await self._execute_batch(bwc, cmd, ops, namespaces)
                result = raw_result
                ...
                # type: ignore[arg-type]
                to_send_ops, _ = await self._execute_batch_unack(bwc, cmd, ops, namespaces)
