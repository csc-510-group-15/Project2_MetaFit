...
class _AsyncBulk:
    ...
    async def write_command(
        ...
        try:
            ...
            # type: ignore[arg-type]
            await client._process_response(reply, bwc.session)
            ...
            if isinstance(exc, (NotPrimaryError, OperationFailure)):
                # type: ignore[arg-type]
                await client._process_response(exc.details, bwc.session)
            raise
    ...
    async def unack_write(
        ...
        try:
            ...
            result = await bwc.conn.unack_write(msg, max_doc_size)
            duration = datetime.datetime.now() - bwc.start_time
    ...
    async def _execute_batch(
        ...
        if self.is_encrypted:
            ...
            request_id, msg, to_send = bwc.batch_command(cmd, ops)
            # type: ignore[arg-type]
            result = await self.write_command(bwc, cmd, request_id, msg, to_send, client)
