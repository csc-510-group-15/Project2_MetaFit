...
class AsyncConnection:
    ...
    def __init__(
        ...
        self.compression_context: Union[SnappyContext,
                                        ZlibContext, ZstdContext, None] = None
...
def _create_connection(address: _Address, options: PoolOptions) -> socket.socket:
    ...
    for res in socket.getaddrinfo(host, port, family, socket.SOCK_STREAM):
        ...
        try:
            sock = socket.socket(af, socktype | getattr(
                socket, "SOCK_CLOEXEC", 0), proto)
        except OSError:
        ...
        if HAS_SNI:
            ...
            if _IS_SYNC:
                ...
                if hasattr(ssl_context, "a_wrap_socket"):
                    ...
                    loop = asyncio.get_running_loop()
                    # type: ignore[assignment, misc]
                    ssl_sock = await loop.run_in_executor(None, ssl_context.wrap_socket, sock)
...
class Pool:
    ...
    async def connect(self, handler: Optional[_MongoClientErrorHandler] = None) -> AsyncConnection:
        ...
        conn = AsyncConnection(sock, self, self.address,
                               conn_id)  # type: ignore[arg-type]
        async with self.lock:
