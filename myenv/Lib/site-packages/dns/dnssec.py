...
def make_ds(
    ...
    *validating*, a ``bool``.  If ``True``, then policy is checked in
    ...
    dsrdata = struct.pack("!HBB", key_id(
        key), key.algorithm, algorithm) + digest
...
def _make_rrsig_signature_data(
    ...
    *origin*, a ``dns.name.Name`` or ``None``, the origin to use for relative
    ...
    rrfixed = struct.pack("!HHI", rdataset.rdtype,
                          rdataset.rdclass, rrsig.original_ttl)
...
def make_ds_rdataset(
    ...
    *origin*, a ``dns.name.Name`` or ``None``.  If `key` is a relative name,
    ...
    for algorithm in _algorithms:
        res.extend(dnskey_rdataset_to_cds_rdataset(
...
def _sign_zone_nsec(
    ...
    for name in sorted(txn.iterate_names()):
        ...
        if last_secure is not None:
            _txn_add_nsec(txn, last_secure, name, zone.rdclass,
                          rrsig_ttl, rrset_signer)
        last_secure = name
