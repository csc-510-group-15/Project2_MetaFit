...
try:
    ...
    class ssl:  # type: ignore
        ...
        def create_default_context(cls, *args, **kwargs):
            raise Exception(
                "no ssl support")  # pylint: disable=broad-exception-raised
...
def _matches_destination(af, from_address, destination, ignore_unexpected):
    ...
    if _addresses_equal(af, from_address, destination) or (
        dns.inet.is_multicast(
            destination[0]) and from_address[1:] == destination[1:]
    ):
        return True
...
def https(
    ...
    *http_version*, a ``dns.query.HTTPVersion``, indicating which HTTP version to use.
    ...
    if session:
        ...
        cm = httpx.Client(http1=h1, http2=h2, verify=verify,
                          transport=transport)
    with cm as session:
...
def udp(
    ...
    *ignore_errors*, a ``bool``.  If various format errors or response
    ...
    with cm as s:
        ...
        return r
    assert (
        # help mypy figure out we can't get here  lgtm[py/unreachable-statement]
        False
    )
