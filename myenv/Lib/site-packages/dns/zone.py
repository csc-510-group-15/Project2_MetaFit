...
class Zone(dns.transaction.TransactionManager):
    ...
    map_factory: Callable[[],
                          MutableMapping[dns.name.Name, dns.node.Node]] = dict
    writable_version_factory: Optional[Callable[[], "WritableVersion"]] = None
    immutable_version_factory: Optional[Callable[[
    ], "ImmutableVersion"]] = None
    ...
    def __init__(
        ...
        *relativize*, a ``bool``, determine's whether domain names are
        ...
        self.nodes: MutableMapping[dns.name.Name,
                                   dns.node.Node] = self.map_factory()
    ...
    def to_text(
        ...
        *want_origin*, a ``bool``.  If ``True``, emit a $ORIGIN line at
        ...
        self.to_file(temp_buffer, sorted, relativize,
                     nl, want_comments, want_origin)
    ...
    def verify_digest(
        ...
        digests: Union[dns.rdataset.Rdataset,
                       List[dns.rdtypes.ANY.ZONEMD.ZONEMD]]
        if zonemd:
            digests = [zonemd]
        else:
            assert self.origin is not None
