...
def kendalltau(x, y, use_ties=True, use_missing=False, method='auto',
    ...
    if not xties and not yties and method == 'exact':
        ...
        var_s += (v1 + v2)
        z = (C-D)/np.sqrt(var_s)
        prob = scipy.stats._stats_py._get_pvalue(
            z, distributions.norm, alternative)
    else:
...
def skewtest(a, axis=0, alternative='two-sided'):
    ...
    pvalue = scipy.stats._stats_py._get_pvalue(
        Z, distributions.norm, alternative)
...
def kurtosistest(a, axis=0, alternative='two-sided'):
    ...
    if np.min(n) < 20:
        warnings.warn(
            "kurtosistest only valid for n>=20 ... continuing anyway, n=%i" % np.min(
                n),
            stacklevel=2,
        )
