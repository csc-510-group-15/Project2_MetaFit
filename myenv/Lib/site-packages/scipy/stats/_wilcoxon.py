...
def _wilcoxon_iv(x, y, zero_method, correction, alternative, method, axis):
    ...
    if 0 < d.shape[-1] < 10 and method == "approx":
        warnings.warn(
            "Sample size too small for normal approximation.", stacklevel=2)
...
def _wilcoxon_nd(x, y=None, zero_method='wilcox', correction=True,
    ...
    temp = _wilcoxon_iv(x, y, zero_method, correction,
    ...
    statistic = np.minimum(
        r_plus, r_minus) if alternative == 'two-sided' else r_plus
    z = -np.abs(z) if (alternative ==
                       'two-sided' and method == 'approx') else z
