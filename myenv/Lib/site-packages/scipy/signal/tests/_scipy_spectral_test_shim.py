...
def _stft_wrapper(x, fs=1.0, window='hann', nperseg=256, noverlap=None,
    ...
    fft_mode = cast(
        FFT_MODE_TYPE, 'onesided' if return_onesided else 'twosided')
...
def _istft_wrapper(Zxx, fs=1.0, window='hann', nperseg=None, noverlap=None,
    ...
    # Using cast() to make mypy happy:
    fft_mode = cast(
        FFT_MODE_TYPE, 'onesided' if input_onesided else 'twosided')
    ...
    if len(Pxy.shape) >= 2 and Pxy.size > 0:
        if Pxy.shape[-1] > 1:
            if average == 'median':
                ...
                raise ValueError(
                    f'average must be "median" or "mean", got {average}')
        else:
