...
def zpk2sos(z, p, k, pairing=None, *, analog=False):
    ...
    *Algorithms*
    ...
    if pairing not in valid_pairings:
        raise ValueError(
            f'pairing must be one of {valid_pairings}, not {pairing}')
...
def ellipap(N, rp, rs):
    ...
    if N % 2:
        newp = np.compress(
            abs(p.imag) > EPSILON *
            np.sqrt(np.sum(p * np.conjugate(p), axis=0).real),
            p, axis=-1
        )
...
def gammatone(freq, ftype, order=None, numtaps=None, fs=None):
    ...
    # Calculate IIR gammatone filter
        ...
        if order is not None:
            warnings.warn(
                'order is not used for IIR gammatone filter.', stacklevel=2)
        if numtaps is not None:
            warnings.warn(
                'numtaps is not used for IIR gammatone filter.', stacklevel=2)
