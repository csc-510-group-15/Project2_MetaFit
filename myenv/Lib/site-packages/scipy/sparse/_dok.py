...
class _dok_base(_spbase, IndexMixin, dict):
    ...
    def update(self, val):
        ...
        raise NotImplementedError(
            "Direct update to DOK sparse format is not allowed.")
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, INT_TYPES):
            ...
            raise IndexError(
                'array/slice index for 1d dok_array not yet supported')
    ...
    def conjtransp(self):
        ...
        new._dict = {(right, left): np.conj(val)
                     for (left, right), val in self.items()}
        return new
    ...
    def tocoo(self, copy=False):
        ...
        coords = tuple(np.fromiter(ix, dtype=idx_dtype, count=nnz)
                       for ix in inds)
        A = self._coo_container(
            (data, coords), shape=self.shape, dtype=self.dtype)
