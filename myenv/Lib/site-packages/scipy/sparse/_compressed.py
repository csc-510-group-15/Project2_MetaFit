...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False):
            ...
            if isshape(arg1, allow_1d=is_array):
                ...
                if len(arg1) == 2:
                    ...
                    self.indices = np.array(
                        indices, copy=copy, dtype=idx_dtype)
        ...
        if shape is not None:
            ...
            self._shape = check_shape(self._swap(
                (major_d, minor_d)), allow_1d=is_array)
    ...
    def check_format(self, full_check=True):
        ...
        if (len(self.indptr) != M + 1):
            raise ValueError(
                f"index pointer size {len(self.indptr)} should be {M + 1}")
        if (self.indptr[0] != 0):
    ...
    def multiply(self, other):
        ...
        if issparse(other):
            ...
            # Other is a row.
            if oM == 1 and sN == oN:
                new_other = _make_diagonal_csr(
                    other.toarray().ravel(), is_array)
    ...
    def _matmul_vector(self, other):
        ...
        # output array
        result = np.zeros(M, dtype=upcast_char(
            self.dtype.char, other.dtype.char))
...
#        return self.__class__(val.reshape(new_shape))
    ...
    def _major_index_fancy(self, idx):
        ...
        row_nnz = (self.indptr[indices + 1] -
                   self.indptr[indices]).astype(idx_dtype)
    ...
    def sum_duplicates(self):
        ...
        M, N = self._swap(self._shape_as_2d)
        _sparsetools.csr_sum_duplicates(
            M, N, self.indptr, self.indices, self.data)
    ...
    def resize(self, *shape):
        ...
        if hasattr(self, 'blocksize'):
            ...
            new_M, new_N = self._swap(
                shape if len(shape) > 1 else (1, shape[0]))
            M, N = self._swap(self._shape_as_2d)
