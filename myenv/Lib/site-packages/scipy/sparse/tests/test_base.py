...
class _TestCommon:
    ...
    def test_from_array(self):
        ...
        A = array([[1.0 + 3j, 0, 0],
                   ...
                   [0, 0, 0]])
        assert_array_equal(self.spcreator(A).toarray(), A)
        assert_array_equal(self.spcreator(
    ...
    def test_from_list(self):
        ...
        assert_array_equal(
            self.spcreator(A, dtype='int16').toarray(), array(
                A).astype('int16')
        )
    ...
    def test_matmul(self):
        ...
        if not isinstance(M, sparray):
            assert_array_almost_equal(
                matmul(M, B).toarray(), (M * B).toarray())
            assert_array_almost_equal(
                matmul(M.toarray(), B), (M * B).toarray())
            assert_array_almost_equal(
                matmul(M, B.toarray()), (M * B).toarray())
    ...
    def test_matvec(self):
        ...
        assert_array_almost_equal(
            M@array([1, 2, 3]), dot(M.toarray(), [1, 2, 3]))
        assert_array_almost_equal(
...
class _TestSlicing:
    def test_dtype_preservation(self):
        assert_equal(self.spcreator((1, 10), dtype=np.int16)
                     [0, 1:5].dtype, np.int16)
        assert_equal(self.spcreator((1, 10), dtype=np.int32)
                     [0, 1:5].dtype, np.int32)
        assert_equal(self.spcreator((1, 10), dtype=np.float32)
                     [0, 1:5].dtype, np.float32)
        assert_equal(self.spcreator((1, 10), dtype=np.float64)
...
class TestLIL(sparse_test_class(minmax=False)):
    ...
    def test_dot(self):
        ...
        assert_array_equal(A @ A.conjugate().T,
                           (B * B.conjugate().T).toarray())
