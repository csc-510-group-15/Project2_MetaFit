...
class _coo_base(_data_matrix, _minmax_mixin):
        ...
        if isinstance(arg1, tuple):
            ...
            if issparse(arg1):
                ...
                if not is_array:
                    ...
                    if M.ndim != 2:
                        raise TypeError(
                            f'expected 2D array or matrix, not {M.ndim}D')
    ...
    def row(self, new_row):
        if self.ndim < 2:
            raise ValueError(
                'cannot set row attribute of a 1-dimensional sparse array')
    ...
    def resize(self, *shape) -> None:
        ...
        if len(shape) < self.ndim:
            tmp_shape = (
                # Original shape without last axis
    ...
    def tocsc(self, copy=False):
        ...
        if self.nnz == 0:
            ...
            indptr, indices, data, shape = self._coo_to_compressed(
                csc_array._swap)
    ...
    def _matmul_multivector(self, other):
        ...
        for i, other_col in enumerate(other.T):
            coo_matvec(self.nnz, row, col, self.data,
                       other_col, result[i:i + 1])
