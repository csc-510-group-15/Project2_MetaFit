...
class SVDSCommonTests:
    ...
    def test_svd_random_state_3(self, random_state):
        ...
        res1a = svds(A, k, solver=self.solver,
                     random_state=random_state, maxiter=1000)
        res2a = svds(A, k, solver=self.solver,
                     random_state=random_state, maxiter=1000)
...
class Test_SVDS_ARPACK(SVDSCommonTests):
    ...
    def test_svds_input_validation_ncv_1(self, ncv):
        ...
        k = 3
        if ncv in {4, 5}:
            u, s, vh = svds(A, k=k, ncv=ncv,
                            solver=self.solver, random_state=0)
