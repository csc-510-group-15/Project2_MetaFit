...
def _nonneg_int_or_fail(n, var_name, strict=True):
    try:
        ...
        raise err.__class__(
            f"{var_name} must be a non-negative integer") from err
    return n
...
def mathieu_even_coef(m, q):
    ...
    km = int(qm + 0.5*m)
    if km > 251:
        warnings.warn("Too many predicted coefficients.",
...
def lpmn(m, n, z):
    ...
    # Convert to int to maintain backwards compatibility.
    m, n = int(m), int(n)
    if (m < 0):
        m_signbit = True
        m_abs = -m
...
def perm(N, k, exact=False):
    ...
    """
    if exact:
        # for backward compatibility (accepted size 1 arrays)
        N = np.squeeze(N)[()]
        k = np.squeeze(k)[()]
        if not (isscalar(N) and isscalar(k)):
            raise ValueError(
                "`N` and `k` must scalar integers be with `exact=True`.")
