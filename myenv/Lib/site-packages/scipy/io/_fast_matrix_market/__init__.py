...
def _read_body_array(cursor):
    ...
    vals = np.zeros(cursor.header.shape,
                    dtype=_field_to_dtype.get(cursor.header.field))
...
def _read_body_coo(cursor, generalize_symmetry=True):
    ...
    data = np.zeros(cursor.header.nnz,
                    dtype=_field_to_dtype.get(cursor.header.field))
...
def _validate_symmetry(symmetry):
    ...
    if symmetry not in symmetries:
        raise ValueError(
            "Invalid symmetry. Must be one of: " + ", ".join(symmetries))
