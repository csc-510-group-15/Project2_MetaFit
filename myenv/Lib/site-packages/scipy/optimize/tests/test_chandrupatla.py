...
class TestChandrupatla(TestScalarRootFinders):
    ...
    def test_vectorization(self, shape, xp):
        ...
        res = _chandrupatla_root(
            f, xp.asarray(-5.), xp.asarray(5.), args=args_xp)
        ...
        ref_success = xp.reshape(xp.asarray(
            ref_success, dtype=xp_test.bool), shape)
    ...
    def test_maxiter_callback(self, xp):
        ...
        for key in res.keys():
            if key == 'status':
                xp_assert_equal(res[key], xp.asarray(
                    eim._ECONVERR, dtype=xp.int32))
                xp_assert_equal(res2[key], xp.asarray(
                    eim._ECALLBACK, dtype=xp.int32))
    ...
    def test_special_cases(self, xp):
        ...
        a, b = xp.asarray([0.1, 0., 0., 0.1]),  xp.asarray(
            [0.9, 1.0, 0.9, 1.0])
        ...
        res = _chandrupatla_root(
            f, xp.asarray(-1), xp.asarray(1), args=xp.asarray(3))
