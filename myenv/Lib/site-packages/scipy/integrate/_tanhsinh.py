...
def _nsum(f, a, b, step=1, args=(), log=False, maxterms=int(2**20), atol=None,
    ...
    if np.any(i2):
        ...
        tmp = _integral_bound(
            f, a[i2], b[i2], step[i2], args_indirect, constants)
...
def _integral_bound(f, a, b, step, args, constants):
    ...
    tol = _logsumexp((tol, rtol + lb.integral)
                     ) if log else tol + rtol*lb.integral
    ...
    n_steps = np.concatenate(
        [2**np.arange(0, log2maxterms), [maxterms]], dtype=dtype)
    nfev = len(n_steps)
    ks = a2 + n_steps * step2
    fks = f(ks, *args2)
    nt = np.minimum(
        np.sum(fks > tol[:, np.newaxis], axis=-1),  n_steps.shape[-1]-1)
    n_steps = n_steps[nt]
    ...
    if log:
        ...
        E_terms = (left_error, right.error - log_step,
                   fk-log2, fb-log2+np.pi*1j)
