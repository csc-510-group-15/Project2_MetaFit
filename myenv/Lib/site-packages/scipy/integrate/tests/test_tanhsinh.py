...
class TestTanhSinh:
    ...
    def test_flags_preserve_shape(self):
        ...
        res = _tanhsinh(f, [np.inf]*3, [-np.inf]*3,
                        maxlevel=5, preserve_shape=True)
        ref_flags = np.array([0, -2, -3])
        assert_equal(res.status, ref_flags)
    ...
    def test_options_and_result_attributes(self):
        ...
        rtol = np.nextafter(ref.error/ref.integral, -np.inf)
        res = _tanhsinh(f, 0, f.b, rtol=rtol)
        assert self.error(res.integral, f.ref) / \
            f.ref < res.error/res.integral < rtol
        assert res.nfev == f.feval > ref.nfev
        assert f.calls > ref.calls
        assert res.success
        assert res.status == 0
...
class TestNSum:
    ...
    def test_inclusive(self):
        ...
        res = _nsum(lambda k: 1 / k ** 2,
                    [1, 4], np.inf, maxterms=500, atol=0.1)
