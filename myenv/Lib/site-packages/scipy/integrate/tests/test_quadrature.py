...
class TestQMCQuad:
    ...
    def basic_test(self, n_points=2**8, n_estimates=8, signs=np.ones(2)):
        ...
        atol = special.stdtrit(n_estimates-1, 0.995) * \
            res.standard_error  # 99% CI
...
def cumulative_simpson_nd_reference(y, *, x=None, dx=None, initial=None, axis=-1):
    ...
    initial = np.moveaxis(
        initial, axis, -1) if np.ndim(initial) > 1 else initial
...
class TestCumulativeSimpson:
    ...
    def test_nd(self, axis, x_ndim, x_len, i_ndim, dx):
        ...
        ref = cumulative_simpson_nd_reference(
            y, x=x, dx=dx, initial=initial, axis=axis)
    ...
    @pytest.mark.parametrize(('message', 'kwarg_update'), [
        ...
        ("x must be strictly increasing", dict(
            x=[x0, [2, 2, 4, 8]], y=[y0, y0])),
        ("x must be strictly increasing", dict(
            x=[x0, x0, x0], y=[y0, y0, y0], axis=0)),
    ...
    @given(
        y=hyp_num.arrays(
            ...
            elements=st.floats(-10, 10,
                               allow_nan=False).filter(lambda x: abs(x) > 1e-7)
        )
    )
