...
def cumulative_simpson(y, *, x=None, dx=1.0, axis=-1, initial=None):
    ...
    if y.shape[-1] < 3:
        res = cumulative_trapezoid(
            original_y, x, dx=dx, axis=axis, initial=None)
        ...
        x = np.broadcast_to(
            x, y.shape) if x.ndim == 1 else np.swapaxes(x, axis, -1)
        dx = np.diff(x, axis=-1)
        if np.any(dx <= 0):
        ...
        final_dx_shape = tupleset(
            original_shape, axis, original_shape[axis] - 1)
