...
def test_question_route_post_correct_answer(client, mocker):
    ...
    response = client.post('/question/1', data={'options': 'a'}, follow_redirects=True)
    assert response.status_code == 200
    with client.session_transaction() as sess:
        assert sess['marks'] == 0
...
def test_question_route_post_empty_option(client, mocker):
    ...
    mocker.patch.object(mongo.db.questions, 'find_one', return_value=mock_question)
    response = client.post('/question/1', data={'options': ''}, follow_redirects=True)
    assert response.status_code == 200

def test_question_route_post_invalid_option(client, mocker):
    mock_question = {"q_id": 1, "a": "Option A", "b": "Option B", "c": "Option C", "d": "Option D", "ans": "Option A"}
    mocker.patch.object(mongo.db.questions, 'find_one', return_value=mock_question)
    response = client.post('/question/1', data={'options': 'Invalid Option'}, follow_redirects=True)
    assert response.status_code == 200
...
def test_question_route_post_repeated(client, mocker):
    ...
    with client.session_transaction() as sess:
        sess['marks'] = 0
    client.post('/question/1', data={'options': 'Option A'})
    response = client.post('/question/1', data={'options': 'Option A'}, follow_redirects=True)
    assert sess['marks'] == 0 
