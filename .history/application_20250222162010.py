...
def user_profile():
    ...
    if session.get('email'):
        ...
        # Try to fetch an existing profile from the 'profile' collection.
        existing_profile = mongo.db.profile.find_one({'email': email}, {
            'height': 1,
            'weight': 1,
            'target_weight': 1
        })
        # Also, check in the user collection as a fallback.
        ...
        if existing_profile:
            # If a profile already exists, set the form fields from the request data
