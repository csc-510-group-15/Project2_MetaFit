...
def send_reset_email(email, reset_code):
    ...
    try:
        ...
        with smtplib.SMTP_SSL(
            ...
            smtp.sendmail(
                INFO_SENDER, email, em.as_string()
            )
...
@password_reset_bp.route(
    "/forgot_password", methods=['GET', 'POST']
)
def forgot_password():
    if request.method == 'POST':
        email = request.form.get('email')
        # Assumes PyMongo instance stored as app.mongo
        mongo = current_app.mongo
        user = mongo.db.user.find_one({"email": email})
        if user:
            # Generate a random reset code
            reset_code = secrets.token_urlsafe(6)
            # Store the code with a timestamp in a dedicated
            # collection
            ...
            # Redirect to reset page with email as query param
            return redirect(
                url_for(
...
@password_reset_bp.route(
    "/reset_password", methods=['GET', 'POST']
)
def reset_password():
    ...
    if request.method == 'POST':
        ...
        if reset_entry:
            # Optionally: Check if code has expired (e.g. > 1 hour)
